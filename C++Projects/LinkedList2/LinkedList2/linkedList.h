...
public:
   ...
   nodeType<Type> *getCurrent() const; //CUSTOM

   ~linkedListIterator(); //CUSTOM
...
public:
	...
	void divideMid(linkedListType<Type> &sublist); //CUSTOM
...
void linkedListType<Type>::destroyList()
{
	...
	//CUSTOM	delete temp;   //deallocate the memory occupied by temp
	}
...
void linkedListType<Type>::print() const
{
	...
	current = first;    //set current so that it points to 
						//the first node
	while (current != last) //while more data to print //CUSTOM changed NULL to last
	{
		cout << current->info << " ";
		current = current->link;
	}

	cout << current->info << " "; //CUSTOM
...
void linkedListType<Type>::divideMid(linkedListType<Type> &sublist){
	...
	if ((mainLength % 2) == 0){
		...
		this->last = itr.getCurrent(); //middle node
		++itr;
...
void linkedListType<Type>::divideAt(linkedListType<Type>  &secondList, const  Type&  item){
	...
	reCount = 0;
	//end recount of list 1

	//begin recount of list 2
